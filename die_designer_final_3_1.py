...
cone_diameter = st.slider("Cone Opening Diameter (mm)", 0.3, 25.0, 12.0)
...
holes_per_row_list = []
for i in range(number_of_rows):
    holes = st.number_input(f"Number of Holes in Row {i+1}", min_value=1, value=10)
...
st.pyplot(fig)
st.markdown(f"**Channel Length:** {channel_length:.2f} mm")
st.markdown(f"**Cone Length:** {cone_length:.2f} mm")
st.markdown(f"**Cone Width:** {cone_diameter:.2f} mm")
st.markdown(f"**Cone Angle:** {cone_angle_deg:.2f}°")
st.markdown(f"**Final Hole Diameter:** {final_diameter:.2f} mm")
...
for row in range(number_of_rows):
    radius = pcd_values[row] / 2
        for i in range(holes_per_row_list[row]):
            angle = 2 * np.pi * i / holes_per_row_list[row]
