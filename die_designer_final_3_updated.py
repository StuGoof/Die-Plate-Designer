...
cone_diameter = st.slider("Cone Opening Diameter (mm)", 0.3, 25.0, max(final_diameter + 0.1, 0.3))
...
for i in range(number_of_rows):
    cols = st.columns(2)
    with cols[0]:
        pcd = st.slider(f"PCD for Row {i+1} (mm)", 50.0, 600.0, 100.0, step=1.0)
    with cols[1]:
...
fig, ax = plt.subplots(figsize=(1.4, 2.1))
...
ax.set_ylabel("Depth (mm, fontsize=6)")
