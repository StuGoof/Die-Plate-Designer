...
for i in range(number_of_rows):
    ...
    with cols[1]:
        holes = st.number_input(f"Holes in Row {i+1}", min_value=1, value=10, step=1, key=f"holes_{i}")
...
fig, ax = plt.subplots(figsize=(1.5, 2.2), dpi=200)
...
# Annotate values
fontsize = 5
ax.text(cone_diameter/2 + 1, cone_length/2, f"Cone Length: {cone_length:.1f} mm", fontsize=fontsize)
ax.text(cone_diameter/2 + 1, plate_thickness - channel_length/2, f"Channel Length: {channel_length:.1f} mm", fontsize=fontsize)
ax.text(cone_diameter/2 + 1, plate_thickness + 1, f"Cone Angle: {cone_angle_deg:.1f}°", fontsize=fontsize)
ax.text(cone_diameter/2 + 1, -2, f"Final Ø: {final_diameter:.1f} mm", fontsize=fontsize)
ax.text(cone_diameter/2 + 1, -4, f"Cone Ø: {cone_diameter:.1f} mm", fontsize=fontsize)

ax.axis('off')
st.pyplot(fig)

# 3D Visualization
fig3d = plt.figure(figsize=(3, 3), dpi=200)
...
ax3d.set_xlabel("X (mm)", fontsize=6)
ax3d.set_ylabel("Y (mm)", fontsize=6)
ax3d.set_zlabel("Depth (mm)", fontsize=6)
...
ax_ring.set_title("Die Hole Layout", fontsize=8)
